...
class CombinedDataset(Dataset):
    def __init__(self, oct_dataset, ffa_dataset, cfp_dataset):
        ...
        self.ffa_len = len(ffa_dataset)
        self.cfp_len = len(cfp_dataset)
        self.oct_len = len(oct_dataset)
        self.total_len = self.ffa_len + self.cfp_len + self.oct_len
        self.index_list = list(range(self.total_len))
    ...
    def __getitem__(self, idx):
        ...
        if true_idx < self.ffa_len:
            ...
            return self.cfp_dataset[true_idx - self.ffa_len]  #  CFP 
        else:
            return self.oct_dataset[true_idx - self.ffa_len - self.cfp_len]  #  OCT 
...
def process(args):
    ...
    #  OCT 
    dataloaders_OCT = get_loader(dataframes_path=args.dataframes_path_OCT, data_root_path=args.data_root_path,
                                datasets=args.datasets_OCT, balance=args.balance, batch_size=args.batch_size,
    ...
    print("OCT loaded successfully!")

    #  FFA 
    dataloaders_FFA = get_loader(dataframes_path=args.dataframes_path, data_root_path=args.data_root_path_FFA,
    ...
    #  OCT 
    oct_train_data = dataloaders_OCT["train"].dataset  #  OCT  train 
    oct_kd_data = dataloaders_OCT["KD"].dataset  #  OCT  KD 
    print("OCT train&kd loaded successfully!")

    #  FFA 
    ffa_train_data = dataloaders_FFA["train"].dataset  #  FFA train 
    ffa_kd_data = dataloaders_FFA["KD"].dataset  #  FFA  KD 
    print("FFA train&kd loaded successfully!")
    ...
    selected_ffa_kd_data = select_representative_samples(model_past, ffa_kd_data, ratio=sample_ratio, device=device)
    print("ffa kd_data selected successfully!")
    selected_ffa_train_data = select_representative_samples_train(model_past, ffa_train_data, ratio=sample_ratio, device=device)
    print("ffa train_data selected successfully!")

    selected_cfp_kd_data = select_representative_samples(model_past, cfp_kd_data, ratio=sample_ratio, device=device)
    print("cfp kd_data selected successfully!")
...
def main():
    ...
    parser.add_argument('--data_root_path_FFA', default=PATH_RESIZED_DATASETS_FFA)

    parser.add_argument('--dataframes_path', default=PATH_DATAFRAME_PRETRAIN_FFA)                         # pair  Pre-processed data pair
    parser.add_argument('--dataframes_path_OCT', default=PATH_DATAFRAME_PRETRAIN_OCT)                         # pair  Pre-processed data pair
    ...
    parser.add_argument('--datasets_OCT', default=["OCT01_RetinalOCT_C8","OCT03_Large_Dataset_of_Labeled_OCT","OCT04_GAMMA1","OCT06_STAGE1",
                                                "OCT07_STAGE2","OCT08_glaucoma_detection","OCT09_GOALS","OCT11_OIMHS","OCT12_OCTA_500","OCT14_DUKE_DME",
    ...
    parser.add_argument('--out_path', default=PATH_RESULTS_PRETRAIN_OCT+"RetCoP/", help='output path')
